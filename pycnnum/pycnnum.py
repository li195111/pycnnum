...
class MathSymbols:
  """Math symbols used in Chinese for both traditional and simplified Chinese
    ...
    """
  def __init__(self, positive: ChineseMath, negative: ChineseMath,
               point: ChineseMath):
    """
        Args:
...
def cn2num(chinese_string: str,
           numbering_type: str = NUMBERING_TYPES[1]) -> Union[int, float]:
  ...
  def string2symbols(
      chinese_string: str,
      system: NumberingSystem) -> Tuple[List[SymbolType], List[SymbolType]]:
    """String to symbols
  ...
  def refine_symbols(integer_symbols: List[SymbolType],
                     system: NumberingSystem) -> List[SymbolType]:
    """Refine symbols
    ...
    if (isinstance(integer_symbols[0], ChineseNumberUnit)
    ...
    for s in integer_symbols:
      if isinstance(s, ChineseNumberDigit):
        ...
        if s.power > last_power:
          value[:-1] = list(map(lambda v: v * pow(10, s.power),
                                value[:-1]))  # type: ignore
          last_power = s.power
        value.append(0)
    return sum(value)
...
def num2cn(
  ...
  def get_value(value_string: str,
                keep_zeros: bool = True) -> List[SymbolType]:
    ...
    return (get_value(result_string, keep_zeros) + [result_unit] +
  ...
  if (result_symbols[0]
      == system.digits[1]) and (len(result_symbols) > 1) and (getattr(
          result_symbols[1], "power", None) == 1):
    result_symbols = result_symbols[1:]

  # remove trailing units, 1600 -> 一千六, 10600 -> 一萬零六百, 101600 -> 十萬一千六
  if len(result_symbols) > 3 and isinstance(result_symbols[-1],
                                            ChineseNumberUnit):
    if getattr(result_symbols[::-1][2], "power",
               None) == (result_symbols[-1].power + 1):
      result_symbols = result_symbols[:-1]
